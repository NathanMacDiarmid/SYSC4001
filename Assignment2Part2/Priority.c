...
int main( int argc, char *argv[]) {
    ...
    do {
        ...
        if(running == NULL){
            ...
            // Highest priority = lowest PID
            if(ready_list!=NULL){
                temp = ready_list;
                int highest_priority_process = temp->p->pid;
                while (temp != NULL) {
                    if (temp->p->pid < highest_priority_process) {
                        highest_priority_process = temp->p->pid;
                    }
                    temp = temp->next;
                }
                running = ready_list;
                while (running != NULL) {
                    if (running->p->pid == highest_priority_process) {
                        break;
                    } else {
